...
//! Struct that represents a TACL request
struct Request {
...
//! Class that manages the TACL requests
...
private:
	...
	//! \brief Add multiple TACL requests
	//!
...
public:
	//! \brief Generate a TACL request waiting for stream completion
	//!
	//! \param stream The stream to wait
	//! \param bind Whether should be bound to the current task
	static Request *generateRequest(aclrtStream stream, bool bind)
	{
		Request *request = Allocator<Request>::allocate();
		assert(request != nullptr);

		aclError eret;
		eret = aclrtCreateEvent(&request->_event);
		if (eret != ACL_ERROR_NONE) {
			ErrorHandler::fail("Failed in aclrtCreateEvent: ", eret);
		}

		eret = aclrtRecordEvent(request->_event, stream);
		if (eret != ACL_ERROR_NONE) {
			ErrorHandler::fail("Failed in aclrtRecordEvent: ", eret);
		}
	...
	static void checkRequests()
	{
		...
		while (it != _pendingRequests.end()) {
			...
			eret = aclrtQueryEvent(request._event, &status);
			if (eret != ACL_ERROR_NONE) {
				ErrorHandler::fail("Failed in aclrtQueryEvent: ", eret);
			}

			if (status == ACL_EVENT_STATUS_RESERVED) {
				ErrorHandler::fail("Detected a reserved event status");
			}
			...
			if (status == ACL_EVENT_STATUS_COMPLETE) {
				...
				eret = aclrtDestroyEvent(request._event);
				if (eret != ACL_ERROR_NONE) {
					ErrorHandler::fail("Failed in aclrtDestroyEvent: ", eret);
				}
