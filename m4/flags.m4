...
AC_DEFUN([AX_COMPILE_FLAGS], [
	...
	AS_IF([test "$enable_debug_mode" = yes],[
		...
		tacl_CPPFLAGS=""
		tacl_CXXFLAGS="-Wall -Wextra -Wshadow -fvisibility=hidden -O0 -g3"
	],[
		# Debug mode is enabled
		tacl_CPPFLAGS="-DNDEBUG"
		tacl_CXXFLAGS="-Wall -Wextra -Wshadow -fvisibility=hidden -O3"
	])

	AC_SUBST(tacl_CPPFLAGS)
	AC_SUBST(tacl_CXXFLAGS)
